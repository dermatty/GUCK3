...
class TelegramComm:
    ...
    def start(self):
        if not self.active:
            return -1
        self.logger.debug(whoami() + "got telegram config, starting telegram handler")
    ...
    def send_message_all(self, text):
        for c in self.chatids:
            try:
                self.bot.send_message(chat_id=c, text="GUCK3 telegram bot started!")
            except Exception as e:
...
def run_mpcommunicator(cfg, mp_loggerqueue):
